...
export default function SideBar({headleOpen, isOpen}:SideBar) {
    ...
    return (
        <div className='flex' >
            <div
                className={`${isOpen ? `w-72` : `w-20`} fixed duration-300 h-screen bg-white border-1 z-20`}
                // onMouseEnter={() => props.headleOpen(true)}
                // onMouseLeave={() => props.headleOpen(false)}
            >
                <div className='h-28'>
                    {isOpen ? (
                        <div className=' absolute cursor-pointer rounded-full -right-4 top-6 bg-white z-50' onClick={() => headleOpen(false)}>
